...
$(document).ready(function() {
  ...
  var gaugeOptions = {
    ...
    pane: {
      ...
      background: {
        backgroundColor:
          (Highcharts.theme && Highcharts.theme.background2) || "#FFFFFF",
        innerRadius: "60%",
        outerRadius: "100%",
        shape: "arc"
      }
      ...
      series: [
        {
          ...
          dataLabels: {
            format:
              '<div style="text-align:center"><span style="font-size:28px;color:' +
  ...
  var chartCache = Highcharts.chart(
    "container-cache",
    Highcharts.merge(gaugeOptions, {
      yAxis: {
        min: 0,
      ...
      series: [
        {
          ...
          dataLabels: {
            format:
              '<div style="text-align:center"><span style="font-size:12px;color:' +
              ((Highcharts.theme && Highcharts.theme.contrastTextColor) ||
                "black") +
              '">{y:.1f}</span><br/>' +
  ...
  var chartRAM_real = Highcharts.chart(
    "container-mem-real",
    Highcharts.merge(gaugeOptions, {
      yAxis: {
        min: 0,
  ...
  var chartSWAP = Highcharts.chart(
    "container-swap",
    Highcharts.merge(gaugeOptions, {
      yAxis: {
        min: 0,
  ...
  var chartDisk = Highcharts.chart(
    "container-disk",
    Highcharts.merge(gaugeOptions, {
      yAxis: {
        min: 0,
  ...
  setInterval(function() {
    $.getJSON("/device", function(data) {
      ...
      $("#cpu-temp").text(
        Math.round((data.cpu.temp / 1000) * Math.pow(10, 1)) / Math.pow(10, 1)
      );
      ...
      for (i = 0; i < data.net.count; i++) {
        $("#net-interface-" + i + "-total-in").text(
          bytesRound(parseInt(data.net.interfaces[i].total_in), 2)
        );
        $("#net-interface-" + i + "-total-out").text(
          bytesRound(parseInt(data.net.interfaces[i].total_out), 2)
        );
      }
    ...
    if (window.dashboard != null) {
      ...
      if (window.dashboard_old != null) {
        ...
        if (window.dashboard_old.net.count > 0) {
          for (i = 0; i < window.dashboard_old.net.count; i++) {
            if (chartNetInterfaces[i].series[0].data.length >= 30) {
              chartNetInterfaces[i].series[0].addPoint(
                parseInt(window.dashboard.net.interfaces[i].total_in) -
                  parseInt(window.dashboard_old.net.interfaces[i].total_in),
                true,
                true
              );
            } else {
            ...
            if (chartNetInterfaces[i].series[1].data.length >= 30) {
              chartNetInterfaces[i].series[1].addPoint(
                parseInt(window.dashboard.net.interfaces[i].total_out) -
                  parseInt(window.dashboard_old.net.interfaces[i].total_out),
                true,
                true
              );
            } else {
        ...
        var idle_diff =
          parseInt(window.dashboard.cpu.stat.idle) -
        ...
        if (chartCPU) {
          ...
          point.update(
            Math.round((1.0 - idle_diff / used_total) * 100 * Math.pow(10, 1)) /
              Math.pow(10, 1)
          );
        }
...
function bytesRound(number, round) {
  if (number < 0) {
    ...
    number = number / 1024;
    var last =
      Math.round(number * Math.pow(10, round)) / Math.pow(10, round) + "K";
  } else if (number < 1048576000) {
    number = number / 1048576;
    var last =
      Math.round(number * Math.pow(10, round)) / Math.pow(10, round) + "M";
  } else {
    number = number / 1048576000;
    var last =
      Math.round(number * Math.pow(10, round)) / Math.pow(10, round) + "G";
  }
  return last;
}
